  ...
  return (
    <div className="py-10">
      ...
      <div className="flex lg:flex-row justify-between items-center flex-col mt-20">
        ...
        <div className="flex space-x-2 md:mb-1 mt-2 md:mt-0">
          {product.stack?.map((stack: string) => (
            <span
              key={stack}
              className="text-xs  md:text-xs lg:text-xs bg-gray-50 dark:bg-slate-700 px-2 py-1 rounded-sm text-secondary dark:text-gray-300"
            >
              {stack}
            </span>
          ))}
      ...
      {/* <a
        href={product.href}
        target="__blank"
        className="inline-flex items-center gap-1 group/button rounded-full hover:scale-105 focus:outline-none transition ring-offset-gray-900 bg-gray-800 dark:bg-gray-100 text-white dark:text-slate-900 shadow-lg shadow-black/20 sm:backdrop-blur-sm group-hover/button:bg-gray-50/15 group-hover/button:scale-105 focus-visible:ring-1 focus-visible:ring-offset-2 ring-gray-50/60 text-sm font-medium px-4 py-2 mt-auto origin-left"
      >
        Live Preview
        <svg
